    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md" onClick={onClose}>
        <motion.div
          ...
          className="w-full max-w-lg bg-[#050505] border relative overflow-hidden rounded-sm shadow-[0_0_50px_rgba(0,0,0,0.8)]"
          style={{ borderColor: `${themeColor}40` }}
        >
          {/* Top Neon Header */}
          <div className="px-6 py-5 border-b flex justify-between items-center" style={{ borderColor: `${themeColor}20` }}>
          ...
          <div className="p-8 space-y-8 font-space">
            {/* Header / Meta */}
            <div className="flex justify-between items-end border-b border-white/5 pb-4">
              <div>
                <p className="text-[10px] text-white/30 uppercase tracking-[0.3em] font-black">DOCUMENT_TYPE</p>
                <p className="text-xs font-black uppercase italic" style={{ color: themeColor }}>{report.type.replace('_', ' // ')}</p>
              </div>
              <div className="text-right">
                <p className="text-[10px] text-white/30 uppercase tracking-[0.3em] font-black">TIMESTAMP</p>
                <p className="text-xs font-black text-white/80">{new Date(report.created_at).toLocaleString()}</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              {report.content.text && (
                <div className="p-4 bg-white/5 border border-white/10 rounded-sm">
                  <p className="text-sm font-bold text-white/90 whitespace-pre-wrap leading-relaxed" dir="auto">
                    {report.content.text}
                  </p>
                </div>
              )}
              ...
              {report.type === 'deadline_alert' && (
                <div className="space-y-6">
                  <div className="flex flex-col items-center justify-center py-4 border-2 border-dashed border-[#FF0055]/30 bg-[#FF0055]/5 rounded-sm">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="p-4 bg-white/5 border border-white/10 rounded-sm">
                      <p className="text-[9px] font-black tracking-widest uppercase opacity-40 mb-1">PROGRESS</p>
                      <p className="text-2xl font-black text-white">{report.content.progress}%</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              {report.type === 'mission_complete' && (
                <div className="space-y-6">
                   <div className="flex flex-col items-center justify-center py-6 bg-neon-green/10 border border-neon-green/30 rounded-sm" style={{ backgroundColor: `${themeColor}10`, borderColor: `${themeColor}30` }}>
                  ...
                  <div className="p-4 bg-white/5 border border-white/10 rounded-sm text-center w-full">
            ...
            <div className="pt-6 border-t border-white/10 flex justify-between items-center opacity-40">
              <p className="text-[8px] tracking-[0.5em] font-black uppercase italic">CORE_OS // CLASSIFIED_UPLINK</p>
